...
|------|---------|-------------|
| `causes` / `caused_by` | Direct causal relationship | No — expander or manual |
| `influences` | Indirect influence | No — expander or manual |
| `contemporaneous` | Exact same date (year, month, day > 1) | Yes, weight 0.5 |
| `same_era` | Same decade AND same country | Yes, weight 0.5 |
| `precedes` / `follows` | Directional temporal (same decade & country) | Yes, weight 0.5 |
| `same_location` | Matching country + region + city | Yes, weight 0.5 |
| `same_figure` | Overlapping historical figures (excludes generic roles) | Yes, weight 0.5 |
...
| **Edge Cleaner** | Batched removal of bad auto-generated edges (gated by `EDGE_CLEANUP_ENABLED`) |
| **Location Backfill** | Fixes missing/unknown coordinates via LLM (gated by `LOCATION_BACKFILL_ENABLED`) |
