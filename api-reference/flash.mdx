...
Returns a paginated list of timepoints. The `tdf_payload` is deferred to avoid loading 1MB+ JSON blobs per item in the list view.
...
```json
{
  "items": [
    {
      "id": "12345",
      "query": "AlphaGo plays Move 37, Seoul, March 10 2016",
...
Generates SSE events for pipeline progress with real-time streaming. The `disconnect_check` parameter is used to detect client disconnections during the stream generation process.

```json
{
  "query": "Moon landing, July 20 1969",
  "generate_image": true,
...
Credits are refunded on generation failure for synchronous, background, and timeout scenarios.

## Request-Level Timeouts

Request-level timeouts have been added to all LLM-calling endpoints to ensure that the API remains responsive and does not hang indefinitely.

## Chat Sessions

Chat sessions are in-memory only and will be lost on restart. Consider implementing database-backed sessions for persistence.
