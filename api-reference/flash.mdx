...
Mint a JWT pair for an existing user, on behalf of a trusted service. Requires a valid `X-Service-Key` header.

```bash
curl -X POST https://api.timepointai.com/api/v1/auth/service-token \
...
|-----------|------|----------|-------------|
| `user_id` | string | Yes | Flash user UUID to mint a token for |
...
```bash
curl -X GET https://api.timepointai.com/api/v1/content/today \
  -H "X-Service-Key: YOUR_KEY"
```
...
### GET /api/v1/content/weekly-prompt

Weekly creative prompt. Returns null when no prompt is active.

```bash
curl -X GET https://api.timepointai.com/api/v1/content/weekly-prompt \
  -H "X-Service-Key: YOUR_KEY"
```
...
```bash
curl -X GET https://api.timepointai.com/api/v1/content/featured \
  -H "X-Service-Key: YOUR_KEY"
```
