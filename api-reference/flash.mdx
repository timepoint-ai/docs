...
### GET /health
Instant liveness probe. No I/O or external connectivity checks.

```bash
GET /health
```

No auth required. Returns service status and configured provider availability.
...
No auth required. Returns the same structure as `/health`, but `database` and provider statuses reflect actual connectivity checks (with a 5s timeout for DB).
