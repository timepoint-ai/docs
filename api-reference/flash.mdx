...
Consumer access goes through the Gateway, which handles authentication and proxies requests to Flash with an `X-User-ID` header. Flash has `AUTH_ENABLED=false` — it does not perform any authentication itself. It trusts the `X-User-ID` injected by the Gateway, which is forwarded to Clockchain for entity visibility filtering (allowing users to see their own private entities during search, resolution, and grounding).
