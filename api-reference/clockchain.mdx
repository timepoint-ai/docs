...
Instant liveness probe. Returns immediately without database I/O.

```bash
curl https://clockchain.timepointai.com/health
```
...
Deep health check. Verifies database connectivity and returns cached graph statistics.

```bash
curl https://clockchain.timepointai.com/health/deep
```
